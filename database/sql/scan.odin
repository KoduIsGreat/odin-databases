...
set_field :: proc(base: rawptr, offset: uintptr, tid: typeid, val: Value) {
	...
	case i64:
		switch tid {
		case i64:    (^i64)(ptr)^  = v
		case int:    (^int)(ptr)^  = int(v)
		case i32:    (^i32)(ptr)^  = i32(v)
		case i16:    (^i16)(ptr)^  = i16(v)
		case u64:    (^u64)(ptr)^  = u64(v)
		case uint:   (^uint)(ptr)^ = uint(v)
		case u32:    (^u32)(ptr)^  = u32(v)
		case u16:    (^u16)(ptr)^  = u16(v)
		case bool:   (^bool)(ptr)^ = v != 0
		}
	case f64:
		switch tid {
		case f64: (^f64)(ptr)^ = v
		case f32: (^f32)(ptr)^ = f32(v)
		}
	case string:
		if tid == string {
			(^string)(ptr)^ = v
