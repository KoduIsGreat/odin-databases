...
make_error :: proc(db: ^sqlite3) -> sql.Error {
	return sql.Driver_Error{
		code    = int(errcode(db)),
		message = string(errmsg(db)),
	}
}
	...
	for i in 0 ..< len(cols) {
		...
		case SQLITE_INTEGER:
			if is_time {
				...
				if ok {
					dest[i] = time.Time{_nsec = unix_sec * i64(1e9)}
				} else {
					dest[i] = unix_sec
				}
			} else {
...
@(private)
build_columns :: proc(stmt: ^sqlite3_stmt, col_count: int, allocator: mem.Allocator) -> []sql.Column {
...
@(private)
sqlite_open :: proc(driver_data: rawptr, dsn: string, allocator: mem.Allocator) -> (sql.Conn_Handle, sql.Error) {
...
@(private)
sqlite_exec :: proc(handle: sql.Conn_Handle, query_str: string, args: []sql.Value) -> (sql.Result, sql.Error) {
	conn := cast(^Sqlite_Conn)handle
...
@(private)
sqlite_query :: proc(handle: sql.Conn_Handle, query_str: string, args: []sql.Value) -> (sql.Rows_Handle, sql.Error) {
	conn := cast(^Sqlite_Conn)handle
...
@(private)
sqlite_prepare :: proc(handle: sql.Conn_Handle, query_str: string) -> (sql.Stmt_Handle, sql.Error) {
	conn := cast(^Sqlite_Conn)handle
...
@(private)
sqlite_stmt_query :: proc(handle: sql.Stmt_Handle, args: []sql.Value) -> (sql.Rows_Handle, sql.Error) {
	wrapper := cast(^Sqlite_Stmt)handle
